...
<main class="relative isolate overflow-hidden bg-cream text-forest">
	...
	<div class="mx-auto max-w-[1600px] px-5 py-24 sm:px-10 sm:py-32 lg:px-14">
		<Eyebrow tone="strong" class="text-forest/50">Order {order.orderNumber}</Eyebrow>
		<h1
		...
		<div class="mt-16 grid gap-16 lg:grid-cols-[minmax(0,1fr)_minmax(0,24rem)] lg:gap-24">
			<div class="max-w-[46rem]">
					...
					<ul class="mt-5 flex list-none flex-col p-0">
						{#each order.lines as line (line.sku)}
							<li class="flex justify-between gap-6 border-b border-forest/10 py-5">
								<div>
									<p class="text-sm text-forest">{line.name}</p>
									...
									{#if line.pieceNumber !== null}
										...
										<p class="mt-2 text-[10px] tracking-[0.28em] text-gold uppercase">
								...
								<p class="shrink-0 text-sm text-forest tabular-nums">
				...
				<section class="mt-14" aria-labelledby="ship-heading">
					...
					{#if order.ship.notes}
						<div class="mt-6 border-l-2 border-forest/20 pl-4">
							...
							<p class="mt-2 text-sm leading-relaxed text-forest/75">{order.ship.notes}</p>
						</div>
					{/if}
				</section>
				...
				{#if order.trackingRef}
					<section class="mt-14" aria-labelledby="tracking-heading">
						<h2
							id="tracking-heading"
							class="text-[10px] tracking-[0.28em] text-forest/60 uppercase"
						>
							Tracking
						</h2>
						<p class="mt-4 text-sm text-forest/80">
				...
				<section class="mt-14 border-t border-forest/15 pt-8" aria-labelledby="returns-heading">
					...
					<p class="mt-4 max-w-[60ch] text-sm leading-relaxed text-forest/75">{RETURNS_WORDING}</p>
					<p class="mt-5 text-sm text-forest/70">
						Write to
			...
			<div class="flex flex-col gap-8 lg:sticky lg:top-28 lg:self-start">
				<section aria-labelledby="status-heading">
					<h2 id="status-heading" class="text-[10px] tracking-[0.28em] text-forest/60 uppercase">
						Status
					</h2>
					<p class="mt-4 text-sm tracking-[0.18em] text-forest uppercase">{shortLabel}</p>
					<p class="mt-2 text-[10px] tracking-[0.2em] text-forest/50 uppercase">
						Placed {longDate.format(order.placedAtMs)}
					</p>
				</section>

				<section class="border-t border-forest/15 pt-6" aria-labelledby="total-heading">
					<h2 id="total-heading" class="text-[10px] tracking-[0.28em] text-forest/60 uppercase">
						Total
					</h2>
					...
					<p class="mt-3 text-[10px] tracking-[0.2em] text-forest/50 uppercase">
						{BUSINESS_NAME}
					</p>
				</section>
