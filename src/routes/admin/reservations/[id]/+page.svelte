...
<div class="mt-6 flex flex-wrap items-center gap-4">
	...
	{/if}
	<p class="text-xs text-stone-500">{r.sku} · size {r.size}</p>
	{#if r.orderId}
		<a
	...
	<div class="flex flex-col gap-14">
		...
		<section class="flex flex-col gap-6">
			...
			<dl class="grid gap-px bg-white/10 sm:grid-cols-3">
				<div class="flex flex-col gap-2 bg-forest/40 px-5 py-6">
					<dt class="text-[10px] tracking-[0.24em] text-stone-400 uppercase">Locked price</dt>
					<dd class="text-xl text-cream tabular-nums">{formatInr(r.lockedPrice)}</dd>
				</div>
				<div class="flex flex-col gap-2 bg-forest/40 px-5 py-6">
					<dt class="text-[10px] tracking-[0.24em] text-stone-400 uppercase">Deposit</dt>
					<dd class="text-xl text-cream tabular-nums">{formatInr(r.deposit)}</dd>
				</div>
				<div class="flex flex-col gap-2 bg-forest/40 px-5 py-6">
					<dt class="text-[10px] tracking-[0.24em] text-stone-400 uppercase">Balance</dt>
				...
				{#each r.timeline as step, index (index)}
					<li class="relative pb-8 last:pb-0">
						<span
							...
							aria-hidden="true"
						></span>
						<p class="text-sm text-cream">{step.label}</p>
						<p class="mt-1 text-xs text-stone-500 tabular-nums">{shortDateTime(step.at)} IST</p>
						{#if step.detail}
							<p class="mt-2 max-w-[42rem] text-sm leading-relaxed text-stone-400">{step.detail}</p>
						{/if}
					</li>
				{/each}
			</ol>
		...
		<section class="flex flex-col gap-6 border-t border-white/10 pt-10">
			...
			{#if canRequestBalance}
				<form method="POST" action="?/balanceLink" class="flex flex-wrap items-end gap-6">
					<div class="flex w-32 flex-col gap-2">
						<label for="days" class="text-[10px] tracking-[0.2em] text-stone-400 uppercase">
							Days to pay
						</label>
						<input
							id="days"
			...
			{#if r.balanceDueBy !== null && canRequestBalance}
				<p class="text-sm {overdue ? 'text-cream' : 'text-stone-400'} tabular-nums">
	...
	<aside class="flex flex-col gap-12">
		<section class="flex flex-col gap-4">
			...
			{#if r.payments.length === 0}
				<p class="text-sm text-stone-500">Nothing captured yet.</p>
			{:else}
		...
		{#if r.refunds.length > 0}
			<section class="flex flex-col gap-4">
				<h2 class="text-[10px] tracking-[0.28em] text-stone-400 uppercase">Refunds</h2>
				<ul class="flex flex-col">
					{#each r.refunds as refund (refund.id)}
						<li class="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 py-4">
							<span class="flex items-center gap-3">
								<StatePill label={humanise(refund.state)} tone="outline" />
								<span class="text-sm text-stone-300">{humanise(refund.reason)}</span>
							</span>
							<span class="text-sm text-cream tabular-nums">{formatInr(refund.amount)}</span>
							<span class="w-full text-xs text-stone-500 tabular-nums">
		...
		<section class="flex flex-col gap-4 border-t border-white/10 pt-8">
			<h2 class="text-[10px] tracking-[0.28em] text-stone-400 uppercase">
				Cancellation rule as shown
			</h2>
			<p class="text-sm leading-relaxed text-stone-400">{r.cancellationRule}</p>
