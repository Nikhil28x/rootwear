			...
			{#each data.orders as order (order.id)}
				<tr class="text-stone-200">
					<th scope="row" class="border-b border-white/5 px-4 py-3 text-left font-normal">
						...
						{#if order.isPreOrder}
							<span class="mt-1 block text-[10px] tracking-[0.18em] text-gold uppercase">
								Pre-order
							</span>
						{/if}
					</th>
					...
					<td class="border-b border-white/5 px-4 py-3 text-sm">{order.email}</td>
					<td class="border-b border-white/5 px-4 py-3 text-sm">
						{order.shipTo.city}, {order.shipTo.state}
						<span class="block text-xs text-stone-500 tabular-nums">{order.shipTo.pincode}</span>
					</td>
					...
					<td class="border-b border-white/5 px-4 py-3 text-xs text-stone-400">
						{#if order.trackingRef}
							{order.courierName}
							<span class="block text-stone-500 tabular-nums">{order.trackingRef}</span>
						{:else}
