...
<div class="no-print">
	...
	<p class="mt-8 text-sm text-stone-400 tabular-nums">
		...
		<article class="packing-card grid gap-8 border border-white/15 p-6 sm:grid-cols-[minmax(0,1fr)_minmax(0,18rem)]">
			<div class="flex flex-col gap-5">
				<div class="flex flex-wrap items-baseline justify-between gap-3">
					<p class="text-sm tracking-[0.2em] text-cream uppercase tabular-nums">
						{String(index + 1).padStart(2, '0')} · {entry.orderNumber}
					</p>
					{#if entry.isPreOrder}
						<p class="text-[10px] tracking-[0.2em] text-gold uppercase">
					...
					{#each entry.lines as line, lineIndex (lineIndex)}
						<li class="flex flex-wrap items-baseline justify-between gap-4 border-b border-white/10 pb-3">
							<span class="text-sm text-stone-200">
								{line.name}
								<span class="ml-3 text-xs tracking-[0.16em] text-stone-500 uppercase">
									{line.sku}
								</span>
							</span>
							<span class="text-sm text-cream tabular-nums">
				...
				{#if entry.notes}
					<p class="border-l-2 border-gold px-4 py-2 text-sm leading-relaxed text-stone-300">
						Customer note: {entry.notes}
					</p>
				{/if}

				<p class="text-xs text-stone-500">{RETURNS_SHORT}</p>
			</div>
			...
			<div class="address-label flex flex-col justify-between gap-6 border border-dashed border-white/25 p-5">
				<address class="text-sm leading-relaxed text-stone-100 not-italic">
				...
				<p class="text-[10px] leading-relaxed tracking-[0.14em] text-stone-500 uppercase">
