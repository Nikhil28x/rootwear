		...
		<div class="mx-auto max-w-[1600px] px-5 py-10 sm:px-10 lg:px-14">
			<div class="flex flex-col gap-10 lg:flex-row lg:gap-14">
				...
				<aside class="admin-sidebar lg:w-56 lg:shrink-0">
					<div class="flex flex-col gap-8 lg:sticky lg:top-24">
						<div class="flex flex-col gap-2 border-b border-white/10 pb-6">
							<p class="wordmark text-sm tracking-[0.22em] text-cream uppercase">Rootwear</p>
							<p class="text-[10px] tracking-[0.28em] text-stone-500 uppercase">Operations</p>
						</div>
							...
							<ul class="flex flex-col">
								{#each nav as item (item.href)}
									<li>
										...
										>
											<span class="text-[11px] tracking-[0.2em] uppercase">{item.label}</span>
											<span class="text-[11px] leading-snug text-stone-500">{item.blurb}</span>
										</a>
									</li>
								{/each}
							</ul>
						...
						<div class="flex flex-col gap-3 border-t border-white/10 pt-6">
							<p class="text-[10px] tracking-[0.24em] text-stone-500 uppercase">Signed in</p>
							<p class="text-xs break-words text-stone-300">{actor.email}</p>
							<p class="text-[10px] tracking-[0.2em] text-gold uppercase">{actor.role}</p>
							{#if actor.role === 'layout'}
								<p class="text-[11px] leading-snug text-stone-500">
							...
							<form method="POST" action="/admin/logout">
								<button
									type="submit"
									class="mt-1 w-full border border-white/25 px-4 py-3 text-[10px] tracking-[0.2em] text-stone-300 uppercase transition hover:bg-white hover:text-black"
								>
									Sign out
								</button>
							</form>
				...
				<main class="min-w-0 flex-1">
					{#if actor.preview}
						<p
							class="admin-banner mb-8 border-l-2 border-gold px-4 py-3 text-xs leading-relaxed text-gold"
						>
					...
					{:else if data.source === 'mock'}
						<p class="admin-banner mb-8 border-l-2 border-gold px-4 py-3 text-xs leading-relaxed text-gold">
