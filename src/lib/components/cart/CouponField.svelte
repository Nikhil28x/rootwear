...
<section class="border-t border-forest/15 py-6" aria-labelledby="coupon-heading">
	<h3 id="coupon-heading" class="text-[10px] tracking-[0.28em] text-forest/60 uppercase">
		Discount code
	</h3>
		...
		<div class="mt-4 flex flex-wrap items-center justify-between gap-4">
			...
			<form method="POST" action="/cart?/removeCoupon" use:enhance>
				<button
					type="submit"
					disabled={busy}
					class="text-[10px] tracking-[0.2em] text-forest/60 uppercase underline-offset-4 transition hover:text-forest hover:underline disabled:opacity-40"
				>
					Remove
				</button>
			</form>
		...
		<form method="POST" action="/cart?/applyCoupon" use:enhance class="mt-4 flex items-end gap-3">
			<div class="flex flex-1 flex-col gap-2">
				...
				<input
					...
					class="w-full border-b border-forest/25 bg-transparent px-0 py-2 text-sm tracking-[0.18em] text-forest uppercase placeholder:tracking-normal placeholder:text-forest/35 placeholder:normal-case focus:border-forest focus:outline-none"
				/>
			</div>
			<button
				type="submit"
				disabled={busy}
				class="border border-forest/30 px-5 py-2 text-[10px] tracking-[0.2em] text-forest uppercase transition hover:bg-forest hover:text-cream disabled:opacity-40"
			>
				Apply
			</button>
		</form>
	...
	{#if message}
		...
		<p id="coupon-message" role="status" class="mt-3 text-xs leading-relaxed text-alert">
			{message}
		</p>
	{/if}
</section>
