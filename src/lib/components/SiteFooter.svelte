	...
	<div class="mx-auto flex max-w-[1600px] flex-col gap-12">
		<div class="grid gap-10 sm:grid-cols-2 lg:grid-cols-4">
			<div class="flex flex-col gap-3">
				...
				<p class="text-xs leading-relaxed">Established in Process.<br />Built from the ground up.</p>
			</div>

			<div class="flex flex-col gap-3">
				<p class="text-[10px] tracking-[0.22em] text-stone-500 uppercase">Information</p>
			...
			<div class="flex flex-col gap-3">
				<p class="text-[10px] tracking-[0.22em] text-stone-500 uppercase">Support</p>
				<ul class="flex flex-col gap-2 text-xs">
			...
			<div class="flex flex-col gap-3">
				<p class="text-[10px] tracking-[0.22em] text-stone-500 uppercase">{BUSINESS_NAME}</p>
				<address
					class="text-xs leading-relaxed not-italic {isPlaceholder(BUSINESS_ADDRESS)
						? 'text-alert-light'
						: ''}"
				>
				...
				{#if GST_POSITION.registered}
					<p class="text-xs">GSTIN {GST_POSITION.gstin}</p>
				{/if}
			</div>
		</div>

		<div
			class="flex flex-col gap-3 border-t border-white/10 pt-7 text-[10px] tracking-[0.16em] uppercase sm:flex-row sm:items-center sm:justify-between"
		>
